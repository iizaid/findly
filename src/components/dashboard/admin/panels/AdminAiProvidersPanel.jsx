...
const PROVIDERS = [
  { id: 'gemini', label: 'Gemini', compatible: false },
  { id: 'openai', label: 'OpenAI', compatible: true },
  { id: 'anthropic', label: 'Anthropic', compatible: false },
  { id: 'deepseek', label: 'DeepSeek', compatible: true },
  { id: 'kimi', label: 'Kimi', compatible: true },
  { id: 'qwen', label: 'Qwen', compatible: true },
];

const statusClass = (status) => {
  if (status === 'configured') return 'bg-emerald-50 text-emerald-700';
  if (status === 'degraded' || status === 'misconfigured') return 'bg-amber-50 text-amber-700';
...
const SecretModal = ({ provider, mode, onClose, onSaved }) => {
  ...
  const submit = async () => {
    ...
    try {
      if (isDelete) {
        await apiRequest(`/api/admin/ai/providers/${provider.provider}/secret`, {
          method: 'DELETE',
          ...
          <>
            ...
            {!isDelete && (
              <>
                ...
                <label className="mb-4 block">
                  <span className="mb-1.5 block text-[13px] font-semibold text-black">Model</span>
                  <input
                    type="text"
                    value={model}
...
const AdminAiProvidersPanel = ({ currentUser }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await apiRequest('/api/admin/ai/providers');
      setState({ status: 'ready', data: response.data });
    } catch (error) {
      setState({ status: 'error', message: getErrorMessage(error, 'Could not load AI providers.') });
    }
  }, []);
  ...
  const providerRows = useMemo(() => {
    ...
    return PROVIDERS.map((provider) => ({ ...provider, ...(byProvider.get(provider.id) || { provider: provider.id, status: 'missing', source: 'missing' }) }));
  }, [state.data]);

  const testProvider = async (provider) => {
    setTesting(provider.provider);
    try {
      await apiRequest(`/api/admin/ai/providers/${provider.provider}/test`, {
        method: 'POST',
  ...
  if (!isRoot) {
    return (
      <div className="rounded-[24px] border border-black/[0.04] bg-white p-6 shadow-sm">
        <p className="text-sm font-bold text-secondary">ROOT access is required.</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-[24px] border border-black/[0.04] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary">ROOT Control</p>
            <h3 className="mt-2 text-2xl font-extrabold tracking-tight text-black">AI Providers</h3>
            <p className="mt-2 max-w-2xl text-[13px] font-semibold leading-6 text-secondary">
              Manage dashboard-encrypted provider keys. Environment keys remain server-side and are never shown here.
            </p>
          </div>
          <button
            type="button"
            ...
            className="inline-flex h-10 items-center gap-2 rounded-xl border border-black/[0.08] bg-white px-4 text-[12px] font-bold text-black hover:bg-black/[0.03] disabled:opacity-50"
          >
        ...
        {!state.data?.secretManagementConfigured && (
          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-800">
            <p className="text-[13px] font-bold">Dashboard secret management is not configured on this server.</p>
          </div>
        )}
        ...
        {providerRows.map((provider) => (
          <div key={provider.id} className="rounded-[22px] border border-black/[0.04] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-3 text-[12px] font-semibold text-secondary sm:grid-cols-2">
              <p>Model: <span className="font-bold text-black">{provider.model || 'Not set'}</span></p>
              <p>Base URL: <span className="font-bold text-black">{provider.baseUrlConfigured ? 'Configured' : 'Not configured'}</span></p>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => setModal({ mode: 'save', provider })}
                disabled={!state.data?.secretManagementConfigured}
                className="inline-flex h-9 items-center gap-2 rounded-xl bg-black px-3 text-[12px] font-bold text-white hover:bg-black/80 disabled:cursor-not-allowed disabled:opacity-40"
              >
                <KeyRound size={14} />
                Add / Replace key
              </button>
              <button
                ...
                disabled={testing === provider.provider || provider.source !== 'dashboard'}
                className="inline-flex h-9 items-center gap-2 rounded-xl border border-black/[0.08] bg-white px-3 text-[12px] font-bold text-black hover:bg-black/[0.03] disabled:cursor-not-allowed disabled:opacity-40"
              >
                <Zap size={14} />
              ...
              <button
                ...
                disabled={provider.source !== 'dashboard'}
                className="inline-flex h-9 items-center gap-2 rounded-xl border border-red-100 bg-red-50 px-3 text-[12px] font-bold text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40"
              >
                <Trash2 size={14} />
                Delete
              </button>
...
export default AdminAiProvidersPanel;
