...
const AdminLiveActivityPanel = () => {
  ...
  const toolbar = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <select value={filters.category} onChange={(e) => setFilters((f) => ({ ...f, category: e.target.value }))} className="h-9 rounded-xl border border-black/[0.08] bg-[#FAFAF9] px-3 text-sm font-semibold text-black outline-none">
      ...
      <select value={filters.severity} onChange={(e) => setFilters((f) => ({ ...f, severity: e.target.value }))} className="h-9 rounded-xl border border-black/[0.08] bg-[#FAFAF9] px-3 text-sm font-semibold text-black outline-none">
        <option value="">All Levels</option>
        <option value="info">Info</option>
        <option value="warning">Warning</option>
        <option value="critical">Critical</option>
      </select>
      <button type="button" onClick={() => refresh(filters)} className="flex h-9 w-9 items-center justify-center rounded-xl border border-black/[0.08] bg-white text-black/50 hover:bg-black/5 hover:text-black transition-colors" aria-label="Refresh">
        <RefreshCw size={14} />
      </button>
    </div>
  );
